...
def construct_synthetic_null(adata, save_path):
    adata2 = ad.AnnData(
        X=adata.layers["counts"].toarray(), obs=adata.obs, var=adata.var
    )
...
def call_de(
    ...
    if correct:
        if (
            PairedData.yuen_t_test(
                x=p_table["pval_trafo_data"],
                y=p_table["pval_trafo_null"],
                alternative="greater",
                paired=True,
                tr=0.1,
            ).rx2("p.value")
    ...
    if threshold == "BC":
        for t in c_abs:
            emp_fdp.append(
                np.min(
                    [
                        (1 / nnull + 1 / nnull * np.sum(contrastScore <= -t))
                        / np.sum(contrastScore >= t),
                        1,
                    ]
                )
            )
        ...
        for t in c_abs:
            emp_fdp.append(
                np.min(
                    [
                        (1 / nnull * np.sum(contrastScore <= -t))
                        / np.sum(contrastScore >= t),
                        1,
                    ]
                )
...
def generate_nb_data_copula(
    ...
    if auto_dist:
        if (
            ("est_overdisp" not in adata.var.columns)
            or ("est_mean" not in adata.var.columns)
            or ("est_zero_inflation" not in adata.var.columns)
        ):
        ...
        if ("nb_overdisp" not in adata.var.columns) or (
            "nb_mean" not in adata.var.columns
        ):
            nb.estimate_overdisp_nb(adata, layer="counts", flavor=nb_flavor)
    ...
    if R_est is None:
        ...
        if auto_dist:
            F = np.array(
                [
                    dist_cdf_selector(
                        X[:, j], means.iloc[j], overdisps.iloc[j], zinfs.iloc[j]
                    )
                    for j in range(p)
                ]
            ).T
            F1 = np.array(
                [
                    dist_cdf_selector(
                        X[:, j] + 1, means.iloc[j], overdisps.iloc[j], zinfs.iloc[j]
                    )
                    for j in range(p)
                ]
            ).T
            ...
            F = np.array(
                [nbinom.cdf(X[:, j], r.iloc[j], q.iloc[j]) for j in range(p)]
            ).T
            F1 = np.array(
                [nbinom.cdf(X[:, j] + 1, r.iloc[j], q.iloc[j]) for j in range(p)]
            ).T
    ...
    Z = rng.multivariate_normal(
        mean=np.zeros(new_data_shape[1]),
        cov=R_est,
        size=new_data_shape[0],
        method="eigh",
        check_valid="warn",
    )
    Z_cdf = norm.cdf(Z)
    if auto_dist:
        Y_gen = np.array(
            [
                dist_ppf_selector(
                    Z_cdf[:, j], means.iloc[j], overdisps.iloc[j], zinfs.iloc[j]
                )
        ...
        Y_gen = np.array(
            [
                nbinom.ppf(Z_cdf[:, j], r.iloc[j], q.iloc[j])
                for j in range(new_data_shape[1])
            ]
        ).T
...
def schaefer_strimmer(X, use_corr=False):
    ...
    lambda_corr = np.min(
        (
            1,
            (np.sum(var_s_st) - np.sum(np.diag(var_s_st)))
...
def select_covariance_scaling(
    adata, cor_cutoff=0.1, min_scale=1, max_scale=2, maxiter=20, rng_seed=1234
):

    data_gen_noscale, R_est_noscale = generate_nb_data_copula(
        adata,
        rng_seed=rng_seed,
        nb_flavor="statsmod_auto",
        auto_dist=True,
        correct_var=True,
        return_R=True,
        corr_factor=1,
    ...
    def opt_fun(factor):
        ...
        data_null_gen2, R_est_new = generate_nb_data_copula(
            adata,
            rng_seed=rng_seed,
            nb_flavor="statsmod_auto",
            auto_dist=True,
        ...
        cor_orig = schaefer_strimmer(
            data_gene_nonzero.layers["counts"].toarray(), use_corr=True
        )
    ...
    xmin, fval, funcalls = golden(
        opt_fun, brack=(min_scale, max_scale), full_output=True, maxiter=maxiter
    )
