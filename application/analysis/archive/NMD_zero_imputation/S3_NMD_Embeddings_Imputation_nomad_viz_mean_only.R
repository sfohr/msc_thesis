...
path <- "./data/gene_pair_df_4_viz_200genes_low-medhigh_mean_only.csv"
...
pair_diffs <- d_agg |> 
  ...
  mutate(
    median_diff = median_expression_low - median_expression_medium_high,
    mean_diff = mean_expression_low - mean_expression_medium_high,
    
    median_negative_expression_low = tidyr::replace_na(median_negative_expression_low, 0.0),
    median_negative_expression_medium_high = tidyr::replace_na(median_negative_expression_medium_high, 0.0),
    median_neg_diff = median_negative_expression_low - median_negative_expression_medium_high,
    
    mean_negative_expression_low = tidyr::replace_na(mean_negative_expression_low, 0.0),
    mean_negative_expression_medium_high = tidyr::replace_na(mean_negative_expression_medium_high, 0.0),
    mean_neg_diff = mean_negative_expression_low - mean_negative_expression_medium_high,
    share_negative_diff = share_negative_low - share_negative_medium_high,
    n_negative_diff = n_negative_low - n_negative_medium_high) |> 
  ungroup()
