...
path <- "./data/gene_pair_df_4_viz_20genes_low-high_mean_sparsity_whereXzero.csv"

d <- read.csv(path) |> filter(!is.na(Expression))
...
# nur die kleinen striche
d_plt <- d |> 
  filter(dist == 0.0, Expression < 0.0) |> 
  left_join(pair_diffs, by="gene_pair_id") |> 
  mutate(
    y_order = dense_rank(desc(median_neg_diff)))
    # y_order = case_when(
...
d_plt |> 
  #mutate(Expression = sqrt(abs(Expression))) |> 
  ggplot(aes(x = Expression, y = factor(y_order), fill = expression_lvl_class)) +
